...
This module provides functionality for parsing documents using Tianshu remote parsing service.

Supported document types:
- PDF documents
- Office documents (.doc, .docx, .ppt, .pptx, .xls, .xlsx) - converted to PDF first
- Image files (.png, .jpg, .jpeg, etc.)
- Text files (.txt, .md)

For Office documents, LibreOffice is required for PDF conversion.
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Parse documents using Tianshu remote parsing service"
    )
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Check Tianshu service availability",
    )

    args = parser.parse_args()

    # Create parser instance
    doc_parser = TianshuParser(tianshu_url=args.tianshu_url)

    # Check installation if requested
    if args.check:
        if doc_parser.check_installation():
            print(f"Tianshu service is available at {args.tianshu_url}")
            return 0
        else:
            print(f"Tianshu service is not available at {args.tianshu_url}")
            return 1
